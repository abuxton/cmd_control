...
# Date        : 12/09/2008
# Author      : Magin George
# Description :  check_log.pl is the perl plugin that runs on the remotehost for monitoring
...
if ((!defined $logfile)||(!defined $rule)||(!defined $servname)){
	&print_usage;
}
...
unless (-e "$rule"){
print "RULE-FILE \"$rule\" does not exists.This may also happen if logfiles parent directory(ies) does not have -x permission for nagios \n";
exit 3;
}

unless (-r "$rule"){
print "RULE-FILE \"$rule\" is not readable by nagios.\n";
exit 3;
}

sub print_usage {
	print 'Usage: check_log.pl -l <logfile> -r <rulesfile> -o <servname>
...
open (RULES,"$rule") or die "cannot find rules file";
...
($string,$errorcode,$rc) = split(/\:\:/,$rule);

	if (($string !~ /^#.*/) && ($string !~ /^$/)){
		@matched = grep(/$string/i ,@tail);
		$nummatched = scalar (@matched);
		if ($nummatched > 0){
		push (@numarray,"$rc\:\:$servname\:\:$host\:\:$errorcode\:\:$string\:\:The string/regex \"$string\" - matched $nummatched time(s)\n");
		}	
	foreach $match (@matched) {			
				$error = "yes";
				push (@array,"$rc\:\:$servname\:\:$host\:\:$errorcode\:\:$string\:\:$match");
				}

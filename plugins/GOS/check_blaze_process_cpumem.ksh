...
# Nov 23 - modifying script so it will work with automated cacti graph generation
...
if [[ $proc == '' ]]; then
   /bin/echo "Blaze process BASE_PORT id is missing"
   print_usage
   exit 3
fi
...
# echo "proc is $proc"
# echo "ps aux | grep \"DBASE_PORT=\"$proc\" | grep -v grep | grep -v "bin/monitor" | grep -v check_process_cpumem.ksh | awk '{PID = $2} {print PID}' | tail -1"
# test=`ps aux | grep "DBASE_PORT=\"$proc\"" | grep -v grep | grep -v "bin/monitor" | grep -v check_process_cpumem.ksh`
# echo "test is $test"
pid=`ps aux | grep "DBASE_PORT=\"$proc\"" | grep -v grep | grep -v "bin/monitor" | grep -v check_process_cpumem.ksh | awk '{PID = $2} {print PID}' | tail -1`
cpu=`top -b -n 1 -p $pid | grep $pid | tail -1 | awk '{CPU += $9} {print CPU}'`
# cpu=`ps aux | grep "DBASE_PORT=\"$proc\"" | grep -v grep | grep -v "bin/monitor" | grep -v check_process_cpumem.ksh | awk '{CPU += $3} {print CPU}' | tail -1`
mem=`ps aux | grep "DBASE_PORT=\"$proc\"" | grep -v grep | grep -v "bin/monitor" | grep -v check_process_cpumem.ksh | awk '{MEM += $4} {print MEM}' | tail -1`
vsz=`ps aux | grep "DBASE_PORT=\"$proc\"" | grep -v grep | grep -v "bin/monitor"| grep -v check_process_cpumem.ksh | awk '{VSZ = $5} {print VSZ}'`
